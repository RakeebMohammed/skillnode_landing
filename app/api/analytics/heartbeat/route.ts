...
export async function POST(req: Request) {
  try {
    ...
    const existing = await Visitor.findOne({ visitorId: body.visitorId }).select("country").lean();
    ...
    const session = await Session.findOne({ sessionId: body.sessionId }).select("startedAt").lean();
