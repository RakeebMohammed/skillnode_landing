...
export default function LeadsPage() {
  ...
  const filtered = useMemo(() => rows.filter(row => (!source || (row.source || "Direct") === source) && JSON.stringify([row.name, row.email, row.phone, row.interest, row.campaign, row.city, row.country]).toLowerCase().includes(query.toLowerCase())), [rows, query, source]);

  function exportCsv() {
    const headers = ["Name", "Email", "Phone", "Interest", "Source", "Medium", "Channel", "Campaign", "Referrer", "City", "Country", "IP", "Created"];
    const lines = [headers, ...filtered.map(r => [r.name, r.email, r.phone || "", r.interest || "", r.source || "Direct", r.medium || "none", r.channel || "Direct", r.campaign || "", r.referrer || "", r.city || "", r.country || "", r.ip || "", new Date(r.createdAt).toISOString()])].map(row => row.map(value => `"${String(value).replaceAll('"', '""')}"`).join(","));
